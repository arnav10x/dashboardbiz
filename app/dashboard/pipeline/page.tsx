...
export default function PipelinePage() {
  ...
  return (
    <div className="flex flex-col h-full fo-page">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 animate-in mobile-compact-page">
        <div className="relative flex flex-wrap items-end justify-between gap-3 mb-5">
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search leads..." className="h-9 w-52 max-w-[44vw] pl-9 pr-3 text-xs outline-none fo-card-2" style={{ color: 'var(--text-primary)' }} />
            </div>
        ...
        {showFilters && (
          <div className="absolute right-0 top-[74px] z-30 w-56 rounded-xl p-3 fo-card mobile-full" style={{ boxShadow: '0 20px 60px rgba(0,0,0,.45)' }}>
        ...
        <div className="grid grid-cols-5 gap-3 mb-5 mobile-card-strip">
          {stats.map((s, idx) => (
        ...
        <div className="grid grid-cols-5 gap-2 mb-5 kanban-mobile-scroll" style={{ display: viewMode === 'board' ? 'grid' : 'none' }}>
          {KANBAN_STAGES.map(stage => {
            const cfg = STAGE_CFG[stage]
            const stageLeads = boardLeads.filter(l => l.stage === stage)
            const sv = stageLeads.reduce((sum, l) => sum + (l.value || 0), 0)
            return (
              <div key={stage} className="fo-card overflow-hidden min-h-[310px] flex flex-col collapsible-mobile-card">
