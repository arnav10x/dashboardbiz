...
function panel(extra: React.CSSProperties = {}): React.CSSProperties { return { background: 'linear-gradient(135deg, var(--glass-bg-a), var(--glass-bg-b))', border: '1px solid var(--glass-border)', borderRadius: 14, boxShadow: 'inset 0 1px 0 var(--glass-inset)', ...extra } }
...
// Local date key — toISOString() would shift dates across midnight in non-UTC timezones.
...
export default function CalendarPage() {
  ...
  // Pad to whole weeks (5 or 6 rows) so late-month days are never cut off.
  ...
  const savePL = async () => {
    ...
    if (user) {
      ...
      const { error } = await supabase.from('cal_entries').upsert({ user_id: user.id, workspace_id: ws?.id || null, entry_date: plModal.date, revenue: Number(plModal.revenue) || 0, expenses: Number(plModal.expenses) || 0, notes: plModal.notes }, { onConflict: 'user_id,entry_date' })
      if (error) { alert(`Could not save entry: ${error.message}`); setSaving(false); return }
      await loadPL()
    }
  ...
  const saveEvent = async () => {
    ...
    if (user) {
      ...
      const { error } = eventModal.id
        ? await supabase.from('calendar_events').update(payload).eq('id', eventModal.id)
        : await supabase.from('calendar_events').insert({ user_id: user.id, ...payload, source: 'manual' })
      if (error) { alert(`Could not save event: ${error.message}`); setSaving(false); return }
  ...
  return (
    <div className="flex min-h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-7 py-7 animate-in">
        ...
        <div className="mb-5 flex gap-8 border-b" style={{ borderColor: 'var(--divider)' }}>
          ...
          <>
            ...
            {scheduleView === 'Agenda' ? (<div className="p-5" style={panel()}>{displayEvents.length ? displayEvents.map(e => <button key={e.id} onClick={()=>openEvent(e.event_date,e)} className="mb-3 flex w-full items-center gap-3 rounded-xl p-3 text-left hover:bg-white/[.035]" style={{border:'1px solid var(--border)'}}><CalendarDays className="h-4 w-4" style={{color:e.color}}/><div className="flex-1"><p className="font-bold">{e.title}</p><p className="text-xs" style={{color:'var(--text-muted)'}}>{e.event_date} {e.all_day ? 'All day' : `${e.start_time?.slice(0,5) || ''} - ${e.end_time?.slice(0,5) || ''}`}</p></div></button>) : <p style={{color:'var(--text-muted)'}}>No events match this filter.</p>}</div>) : scheduleView === 'Month' ? (<div className="p-5" style={panel()}><div className="grid grid-cols-7 gap-2">{Array.from({length: Math.ceil(((new Date(year, month, 1).getDay()+6)%7 + new Date(year, month+1, 0).getDate())/7)*7}, (_,idx)=>{ const first=(new Date(year,month,1).getDay()+6)%7; const day=idx-first+1; return day>0 && day<=new Date(year,month+1,0).getDate() ? new Date(year,month,day) : null }).map((d,idx)=> d ? <button key={dateKey(d)} onDoubleClick={()=>openEvent(dateKey(d))} className="min-h-[90px] rounded-lg p-2 text-left hover:bg-white/[.035]" style={{border:'1px solid var(--border)'}}><p className="text-xs" style={{color:'var(--text-muted)'}}>{d.getDate()}</p>{displayEvents.filter(e=>e.event_date===dateKey(d)).slice(0,2).map(e=><p key={e.id} className="mt-1 truncate rounded px-2 py-1 text-[10px]" style={{background:`${e.color}22`,color:e.color}}>{e.title}</p>)}</button> : <div key={idx} className="min-h-[90px] rounded-lg p-2 opacity-30" style={{border:'1px solid var(--border)'}} />)}</div></div>) : <div className="overflow-hidden" style={panel()}>
              <div className="grid" style={{ gridTemplateColumns: '68px repeat(7,1fr)' }}>
                <div />
                {wDates.map((d, i) => <div key={i} className="border-l py-3 text-center" style={{ borderColor: 'var(--chart-grid)' }}><p className="text-[11px] font-black">{DAYS[i]}<br/><span style={{ color: 'var(--text-secondary)', fontWeight: 500 }}>{MONTHS[d.getMonth()].slice(0,3)} {d.getDate()}</span></p></div>)}
                <div className="border-t pr-2 pt-3 text-right text-xs" style={{ borderColor: 'var(--chart-grid)' }}>all-day</div>
                {wDates.map((d, i) => <div key={i} onDoubleClick={() => openEvent(dateKey(d), { id: '', title: '', event_date: dateKey(d), all_day: true, color: '#23c767' } as CalEvent)} className="border-l border-t p-1" style={{ borderColor: 'var(--chart-grid)', minHeight: 34 }}>{displayEvents.filter(e => e.all_day && e.event_date === dateKey(d)).map(e => <button key={e.id} onClick={() => openEvent(e.event_date, e)} className="mb-1 w-full truncate rounded px-2 py-1 text-[10px] font-bold" style={{ background: `${e.color}26`, border: `1px solid ${e.color}45`, color: e.color }}>{e.title}</button>)}</div>)}
                {HOURS.map(h => <div key={`r-${h}`} className="contents"><div className="border-t pr-2 pt-2 text-right text-xs" style={{ borderColor: 'var(--chart-grid)', color: 'var(--text-secondary)', minHeight: 56 }}>{hourLabel(h)}</div>{wDates.map((d, i) => {
                  const cellDate = dateKey(d)
                  const showNowLine = cellDate === todayKey && h === currentHour
                  return (
                    <div key={`${h}-${i}`} onDoubleClick={() => setEventModal({ date: cellDate, title: '', description: '', start_time: `${String(h).padStart(2, '0')}:00`, end_time: `${String(h + 1).padStart(2, '0')}:00`, all_day: false, color: '#23c767' })} className="relative border-l border-t p-1" style={{ borderColor: 'var(--chart-grid)', minHeight: 56 }}>
                      {showNowLine && (
