      ...
      <div className="flex-1 overflow-y-auto px-7 py-7 animate-in">
        ...
        {activeTab === 'schedule' ? (
          ...
          <>
            ...
            <div className="mt-4 grid gap-4 xl:grid-cols-[.75fr_1.25fr]"><div className="p-5" style={panel()}><h3 className="mb-5 font-black">Profit / Loss Insights</h3>{[`Your highest profit day was ${high ? new Date(high.date+'T12:00:00').toLocaleDateString('en-US',{ month:'short', day:'numeric' }) : 'not logged'} with ${high ? money(high.revenue-high.expenses) : '$0'}.`, `You had ${lossDays} loss days this month.`, `Net profit is ${totalProfit >= 0 ? 'up' : 'down'} this month.`].map((t,i)=><p key={i} className="mb-5 text-sm" style={{ color: 'var(--text-secondary)' }}>{t}</p>)}</div><div className="p-5" style={panel()}><h3 className="mb-3 font-black">Daily Net Profit Trend</h3><div className="h-48"><ResponsiveContainer><AreaChart data={trend}><CartesianGrid stroke="var(--chart-grid)" vertical={false}/><XAxis dataKey="day" tick={{ fill:'#7d837f', fontSize: 10 }} axisLine={false} tickLine={false}/><YAxis tick={{ fill:'#7d837f', fontSize: 10 }} axisLine={false} tickLine={false}/><Tooltip contentStyle={{ background:'var(--tooltip-bg)', border:'1px solid var(--tooltip-border)', borderRadius:8 }}/><Area dataKey="profit" stroke="var(--accent)" fill="var(--accent-muted)" strokeWidth={2}/></AreaChart></ResponsiveContainer></div></div></div>
          </>
        )}
      </div>
