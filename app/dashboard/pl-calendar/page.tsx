    ...
    <div className="flex min-h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-7 py-7 animate-in">
        ...
        <div className="mb-5 flex gap-8 border-b" style={{ borderColor: 'rgba(255,255,255,.08)' }}>
          <button onClick={() => setActiveTab('schedule')} className="pb-3 text-sm font-bold" style={{ color: activeTab === 'schedule' ? '#26d36e' : 'var(--text-secondary)', borderBottom: activeTab === 'schedule' ? '2px solid #26d36e' : '2px solid transparent' }}>Schedule Calendar</button>
          <button onClick={() => setActiveTab('pl')} className="pb-3 text-sm font-bold" style={{ color: activeTab === 'pl' ? '#26d36e' : 'var(--text-secondary)', borderBottom: activeTab === 'pl' ? '2px solid #26d36e' : '2px solid transparent' }}>Profit / Loss Calendar</button>
        </div>

        {activeTab === 'schedule' ? (
          <>
            <div className="mb-3 flex items-center justify-between">
              <div className="flex rounded-lg" style={panel({ overflow: 'hidden' })}>{(['Week','Month','Agenda'] as const).map(v => <button key={v} onClick={() => setScheduleView(v)} className="px-5 py-2 text-xs font-bold" style={{ color: scheduleView === v ? '#26d36e' : 'var(--text-secondary)', background: scheduleView === v ? 'rgba(38,211,110,.10)' : 'transparent' }}>{v}</button>)}</div>
            ...
            <div className="mt-5 grid gap-4 xl:grid-cols-[.9fr_1.6fr]">
              <div className="p-5" style={panel()}><div className="mb-4 flex justify-between"><h3 className="font-black">Connected Calendars</h3><Settings className="h-4 w-4" /></div><div className="mb-3 flex items-center gap-3"><div className="grid h-9 w-9 place-items-center rounded-lg bg-white text-black font-black">G</div><div className="flex-1"><p className="font-bold">Google Calendar</p><p className="text-xs" style={{ color: 'var(--text-muted)' }}>{googleConnected ? 'Connected to your account' : 'Connect to sync real events'}</p></div><button onClick={connectGoogle} className="rounded-lg px-3 py-1 text-xs font-bold" style={{ background: googleConnected ? 'rgba(38,211,110,.12)' : 'rgba(255,255,255,.04)', color: googleConnected ? '#26d36e' : 'white', border: '1px solid rgba(255,255,255,.08)' }}>{googleConnected ? 'Connected' : 'Connect'}</button><MoreVertical className="h-4 w-4" /></div><button onClick={connectGoogle} className="mt-1 flex items-center gap-2 text-sm font-bold" style={{ color: '#26d36e' }}><Plus className="h-4 w-4" /> Connect calendar</button></div>
              <div className="p-5" style={panel()}><div className="mb-4 flex justify-between"><h3 className="font-black">Upcoming Events</h3><button onClick={()=>setScheduleView('Agenda')} className="text-sm font-bold" style={{ color: '#26d36e' }}>View full agenda →</button></div>{displayEvents.filter(e=>!e.all_day).slice(0,5).length ? displayEvents.filter(e=>!e.all_day).slice(0,5).map(e=><button key={e.id} onClick={()=>openEvent(e.event_date,e)} className="mb-3 flex w-full items-center gap-3 text-left"><div className="grid h-8 w-8 place-items-center rounded-lg" style={{ background: `${e.color}20`, color: e.color }}><CalendarDays className="h-4 w-4" /></div><p className="flex-1 font-bold">{e.title}</p><span className="text-sm" style={{ color: 'var(--text-muted)' }}>{e.start_time?.slice(0,5)} – {e.end_time?.slice(0,5)}</span><span className="rounded-md px-2 py-1 text-xs" style={{ color: e.color, background: `${e.color}18` }}>Event</span></button>) : <p className="text-sm" style={{ color: 'var(--text-muted)' }}>No upcoming events yet. Click Add Event or double-click a calendar slot to create one.</p>}</div>
            </div>
          </>
        ) : (
          <>
            <div className="mb-5 grid grid-cols-5 gap-3">{[
              ['Total Net Profit', money(totalProfit), 'This month', totalProfit >= 0 ? '#26d36e' : '#ff4d4d'], ['Highest Day', high ? money(high.revenue - high.expenses) : '$0', high ? new Date(high.date+'T12:00:00').toLocaleDateString('en-US',{ month:'short', day:'numeric', year:'numeric' }) : '', '#26d36e'], ['Lowest Day', low ? money(low.revenue - low.expenses) : '$0', low ? new Date(low.date+'T12:00:00').toLocaleDateString('en-US',{ month:'short', day:'numeric', year:'numeric' }) : '', '#ff4d4d'], ['Profit Days', String(profitDays), `${entryList.length ? Math.round(profitDays / entryList.length * 100) : 0}% of days`, '#26d36e'], ['Loss Days', String(lossDays), `${entryList.length ? Math.round(lossDays / entryList.length * 100) : 0}% of days`, '#ff4d4d']
            ].map(c => <div key={c[0]} className="p-4" style={panel()}><p className="mb-3 text-[11px] font-black uppercase tracking-[.15em]" style={{ color: 'var(--text-muted)' }}>{c[0]}</p><p className="text-2xl font-black" style={{ color: c[3] }}>{c[1]}</p><p className="mt-2 text-sm" style={{ color: 'var(--text-secondary)' }}>{c[2]}</p></div>)}</div>
