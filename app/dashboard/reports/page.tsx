...
function Pill({ href, children, active=false }: { href:string; children: React.ReactNode; active?: boolean }) {
  return <Link href={href} className="px-4 py-2 text-[11px] font-bold rounded-md transition-all hover:scale-[1.03] hover:border-green-500/50" style={{ background: active ? 'linear-gradient(180deg, var(--accent-hover), var(--accent))' : 'var(--bg-raised)', color: active ? 'var(--accent-fg)' : 'var(--text-secondary)', border: active ? '1px solid var(--accent-glow)' : '1px solid var(--border)', textDecoration:'none' }}>{children}</Link>
}
function Metric({ label, value, sub, Icon, progress=0, healthy=true }: any) { return <div className="app-card" style={{ minHeight: 124 }}><div className="app-card-inner flex flex-col" style={{ padding: '15px 16px 14px' }}><div className="flex items-center justify-between gap-2 mb-3"><p className="fo-kicker truncate">{label}</p><div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: healthy ? 'var(--accent-faint)' : 'rgba(255,71,71,.08)', border: healthy ? '1px solid var(--accent-muted)' : '1px solid rgba(255,71,71,.18)' }}><Icon className="w-3.5 h-3.5" style={{ color: healthy ? 'var(--accent)' : '#ff4747' }} /></div></div><p className="text-2xl md:text-[26px] font-black fo-num leading-none truncate" style={{ color:'var(--text-primary)' }}>{value}</p><p className="text-xs mt-1.5 truncate" style={{ color:'var(--text-muted)' }}>{sub}</p><div className="mt-auto pt-4"><div className="fo-soft-line"><span style={{ width:`${progress}%`, background: healthy ? 'linear-gradient(90deg, var(--accent), var(--accent-hover))' : '#ff4747' }} /></div></div></div><div className="app-card-glow" /></div> }
  ...
  return <div className="flex flex-col h-full fo-page"><TopBar title="Reports" workspaceName={workspaceName} hasData={revenue > 0 || completed > 0 || pipelineLeads > 0} showGreeting />
    <div className="flex-1 overflow-y-auto p-7 md:p-8 animate-in"><div className="max-w-[1180px] space-y-6 stagger">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.05fr_1fr] gap-4">
        ...
        <Card className="p-6"><div className="flex justify-between items-center mb-6"><h2 className="text-lg font-bold">Performance Overview</h2><span className="px-3 py-1.5 text-xs rounded-lg" style={{ background:'var(--overlay-faint)', border:'1px solid var(--border)' }}>{range.toUpperCase()}</span></div><div className="space-y-5"><ProgressRow label="Revenue" pct={targetPct} healthy={target ? targetPct >= 60 : revenue > 0}/><ProgressRow label="Outreach" pct={Math.min(100, pipelineLeads * 10)} healthy={pipelineLeads > 0}/><ProgressRow label="Pipeline" pct={conversion} healthy={conversion >= 20}/><ProgressRow label="Execution" pct={taskRate} healthy={taskRate >= 60}/></div></Card>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card className="p-6"><h2 className="text-lg font-bold mb-5">Insights</h2>{[`${revenue > 0 ? `Revenue in this range is ${money(revenue)}.` : 'No revenue logged in this range. Add data to unlock revenue insights.'}`,`${pipelineLeads > 0 ? `You have ${pipelineLeads} pipeline leads and ${won} won deals.` : 'No pipeline leads logged yet.'}`,`${won === 0 ? 'No deals closed yet — focus on follow-up.' : `${won} deals closed. Keep moving leads forward.`}`,`${tasks?.length ? `Task completion rate is ${taskRate}%.` : 'Add tasks to track execution.'}`].map(t=><div key={t} className="flex items-start gap-3 mb-4"><CheckCircle2 className="w-4.5 h-4.5 text-accent flex-shrink-0 mt-0.5" style={{ width: 18, height: 18 }}/><p className="text-sm" style={{ color:'var(--text-secondary)' }}>{t}</p></div>)}<Link href="/dashboard/ai-copilot" className="block text-center text-accent font-bold text-sm mt-6">View full insights <ArrowRight className="inline w-4 h-4" /></Link></Card>
      </div>
    </div></div></div>
}
