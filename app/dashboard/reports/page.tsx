...
function Pill({ href, children, active=false }: { href:string; children: React.ReactNode; active?: boolean }) {
  return <Link href={href} className="px-4 py-2 text-[11px] font-bold rounded-md transition-all hover:scale-[1.03] hover:border-green-500/50" style={{ background: active ? 'linear-gradient(180deg,#35e680,#22bf63)' : 'rgba(255,255,255,.035)', color: active ? '#031008' : 'var(--text-secondary)', border: active ? '1px solid rgba(39,211,110,.7)' : '1px solid var(--border)', textDecoration:'none' }}>{children}</Link>
}
function Metric({ label, value, sub, Icon, progress=0, healthy=true }: any) { return <Card className="p-5 min-h-[150px] transition-all hover:-translate-y-0.5 hover:border-green-500/35 hover:shadow-[0_0_40px_rgba(34,197,94,.08)]"><div className="flex items-start justify-between"><div><p className="fo-kicker mb-5">{label}</p><p className="text-3xl font-black fo-num text-white">{value}</p><p className="text-xs mt-3" style={{ color:'var(--text-muted)' }}>{sub}</p></div><div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: healthy ? 'rgba(39,211,110,.08)' : 'rgba(255,71,71,.08)', border: healthy ? '1px solid rgba(39,211,110,.18)' : '1px solid rgba(255,71,71,.18)' }}><Icon className="w-4 h-4" style={{ color: healthy ? '#22c55e' : '#ff4747' }} /></div></div><div className="fo-soft-line mt-5"><span style={{ width:`${progress}%`, background: healthy ? 'linear-gradient(90deg,#1fc85f,#41e47b)' : '#ff4747' }} /></div></Card> }
function ProgressRow({ label, pct, healthy }: { label:string; pct:number; healthy:boolean }) { return <div className="grid grid-cols-[90px_1fr_46px] items-center gap-4"><span className="text-sm" style={{ color:'var(--text-secondary)' }}>{label}</span><div className="h-1.5 rounded-full overflow-hidden" style={{ background:'rgba(255,255,255,.06)' }}><div className="h-full rounded-full" style={{ width:`${pct}%`, background: healthy ? 'linear-gradient(90deg,#1fc85f,#41e47b)' : '#ff4747' }} /></div><span className="text-sm text-right" style={{ color: healthy ? 'var(--text-secondary)' : '#ff5454' }}>{pct}%</span></div> }
  ...
  return <div className="flex flex-col h-full fo-page"><TopBar title="Reports" workspaceName={workspaceName} hasData={revenue > 0 || completed > 0 || pipelineLeads > 0} showGreeting />
    <div className="flex-1 overflow-y-auto p-7 md:p-8 animate-in"><div className="max-w-[1180px] space-y-6">
      ...
      <div className="grid grid-cols-[1.05fr_1fr] gap-4">
        <Card className="p-6 min-h-[285px]"><div className="flex items-center justify-between mb-4"><div><h2 className="text-lg font-bold">Revenue Over Time <span className="text-xs opacity-50">ⓘ</span></h2><p className="text-3xl font-black fo-num mt-2">{money(revenue)}</p><p className="text-sm mt-2" style={{ color: revenue > 0 ? '#22c55e' : 'var(--text-muted)' }}>{revenueTrendText}</p></div><Link href="/dashboard/period-entry" className="px-3 py-2 text-xs rounded-lg hover:border-green-500/40" style={{ background:'rgba(255,255,255,.04)', border:'1px solid var(--border)', textDecoration:'none', color:'inherit' }}>Add data</Link></div><div className="h-36 relative mt-3">{graph.polyline ? <svg viewBox="0 0 520 150" className="w-full h-full"><defs><linearGradient id="reportRevenue" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#22c55e" stopOpacity=".35"/><stop offset="1" stopColor="#22c55e" stopOpacity="0"/></linearGradient></defs>{[25,60,95,130].map(y=><line key={y} x1="0" x2="520" y1={y} y2={y} stroke="rgba(255,255,255,.07)"/>)}<polygon points={graph.area} fill="url(#reportRevenue)"/><polyline points={graph.polyline} fill="none" stroke="#22c55e" strokeWidth="3"/>{graph.circles.map((c,i)=><circle key={i} cx={c.x} cy={c.y} r="4" fill="#22c55e" />)}</svg> : <div className="h-full flex items-center justify-center text-sm" style={{ color:'var(--text-muted)' }}>Log entries to unlock an accurate chart.</div>}</div></Card>
        <Card className="p-6"><div className="flex justify-between mb-8"><h2 className="text-lg font-bold">Performance Overview</h2><span className="px-3 py-2 text-xs rounded-lg" style={{ background:'rgba(255,255,255,.04)', border:'1px solid var(--border)' }}>{range.toUpperCase()}</span></div><div className="space-y-8"><ProgressRow label="Revenue" pct={targetPct} healthy={target ? targetPct >= 60 : revenue > 0}/><ProgressRow label="Outreach" pct={Math.min(100, pipelineLeads * 10)} healthy={pipelineLeads > 0}/><ProgressRow label="Pipeline" pct={conversion} healthy={conversion >= 20}/><ProgressRow label="Execution" pct={taskRate} healthy={taskRate >= 60}/></div></Card>
      </div>
      <div className="grid grid-cols-2 gap-4">
        <Card className="p-6"><h2 className="text-lg font-bold mb-6">Reports Breakdown</h2>{[{href:'/dashboard/period-entry',i:Briefcase,t:'Financial Summary',d:'Revenue, profit, and margin trends.'},{href:'/dashboard/pipeline',i:ShieldCheck,t:'Pipeline Report',d:'Lead flow, conversion, and win rate.'},{href:'/dashboard/tasks',i:CheckCircle2,t:'Task Performance',d:'Completion rate and productivity.'},{href:'/dashboard/achievements',i:Activity,t:'Activity Report',d:'Daily activity and consistency.'}].map(x=> <div key={x.t} className="flex items-center gap-4 mb-4 rounded-lg p-2 -mx-2 transition-all hover:bg-white/[.035]"><div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background:'rgba(39,211,110,.08)', border:'1px solid rgba(39,211,110,.18)' }}><x.i className="w-4 h-4 text-green-500"/></div><div className="flex-1"><p className="font-bold text-sm">{x.t}</p><p className="text-xs" style={{ color:'var(--text-muted)' }}>{x.d}</p></div><Link href={x.href} className="px-5 py-2 rounded-lg text-sm" style={{ background:'rgba(255,255,255,.035)', border:'1px solid var(--border)', textDecoration:'none', color:'inherit' }}>View</Link></div>)}<Link href="/dashboard/ai-copilot" className="block text-center text-green-500 font-bold text-sm mt-8">Ask AI for a report <ArrowRight className="inline w-4 h-4" /></Link></Card>
        <Card className="p-6"><h2 className="text-lg font-bold mb-8">Insights</h2>{[`${revenue > 0 ? `Revenue in this range is ${money(revenue)}.` : 'No revenue logged in this range. Add data to unlock revenue insights.'}`,`${pipelineLeads > 0 ? `You have ${pipelineLeads} pipeline leads and ${won} won deals.` : 'No pipeline leads logged yet.'}`,`${won === 0 ? 'No deals closed yet — focus on follow-up.' : `${won} deals closed. Keep moving leads forward.`}`,`${tasks?.length ? `Task completion rate is ${taskRate}%.` : 'Add tasks to track execution.'}`].map(t=><div key={t} className="flex items-center gap-4 mb-7"><CheckCircle2 className="w-5 h-5 text-green-500"/><p className="text-sm" style={{ color:'var(--text-secondary)' }}>{t}</p></div>)}<Link href="/dashboard/ai-copilot" className="block text-center text-green-500 font-bold text-sm mt-10">View full insights <ArrowRight className="inline w-4 h-4" /></Link></Card>
      </div>
    </div></div></div>
}
