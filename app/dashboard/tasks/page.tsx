    ...
    if (next) {
      try {
        const res = await fetch('/api/gamification', {
          ...
          body: JSON.stringify({ action: getPriority(task) === 'High' ? 'complete_high_priority_task' : 'complete_task' })
        })
        if (res.ok) {
          const gamification = await res.json()
          window.dispatchEvent(new CustomEvent('founderos:gamification-updated', { detail: gamification }))
        }
      } catch {}
    }
  }
