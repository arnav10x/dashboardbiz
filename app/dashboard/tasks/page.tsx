...
const priorityStyle: Record<Priority, { color: string; bg: string }> = {
  ...
  Low: { color: '#26d36e', bg: 'rgba(38, 211, 110, .10)' },
}
...
function cardStyle(extra: React.CSSProperties = {}): React.CSSProperties {
  return {
    background: 'linear-gradient(135deg, rgba(255,255,255,.043), rgba(255,255,255,.014))',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,.78)' }} onClick={e => e.currentTarget === e.target && onClose()}>
      <div className="w-full max-w-md p-5" style={cardStyle({ background: '#0c0e11' })}>
        ...
        <div className="mt-5 flex gap-2">
          ...
          <button onClick={save} disabled={saving || !title.trim()} className="flex-1 rounded-xl py-2 text-xs font-black disabled:opacity-50" style={{ background: 'linear-gradient(180deg,#31df77,#18b85b)', color: '#031008' }}>{saving ? 'Saving…' : 'Add Task'}</button>
        </div>
      </div>
    </div>
  )
    ...
    <div className="flex min-h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-7 py-7 animate-in">
        <div className="mb-7 flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              ...
              <span className="rounded-md px-2 py-1 text-[10px] font-black uppercase" style={{ color: pct === 100 && totalCount > 0 ? '#26d36e' : '#f5a623', background: pct === 100 && totalCount > 0 ? 'rgba(38,211,110,.08)' : 'rgba(245,166,35,.08)' }}>{pct === 100 && totalCount > 0 ? 'All Done' : `${openTasks.length} Open`}</span>
            </div>
        ...
        <div className="mb-6 flex items-center gap-5">
          <div className="h-2 flex-1 rounded-full" style={{ background: 'rgba(255,255,255,.06)' }}><div className="h-full rounded-full" style={{ width: `${pct}%`, background: '#26d36e', boxShadow: '0 0 18px rgba(38,211,110,.24)' }} /></div>
          <div className="flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-full text-xs font-black" style={{ border: '3px solid #26d36e', color: '#fff' }}>{pct}%</div><span className="font-bold">Complete</span></div>
        </div>

        <div className="mb-4 p-5" style={cardStyle({ minHeight: 220 })}>
          <div className="mb-5 flex items-center justify-between gap-3">
            <div className="flex items-center gap-2 text-[12px] font-black uppercase tracking-[.18em]" style={{ color: '#26d36e' }}><Sparkles className="h-4 w-4" /> AI Tailored Tasks</div>
          ...
          <div className="space-y-3">
            {aiTasks.length > 0 ? aiTasks.map(task => (
              <div key={task.id} className="flex items-center gap-4 rounded-xl px-4 py-4" style={{ background: 'rgba(255,255,255,.028)', border: '1px solid rgba(255,255,255,.06)' }}>
                <button onClick={() => toggleTask(task)} className="grid h-7 w-7 place-items-center rounded-full" style={{ background: task.is_completed ? '#26d36e' : 'transparent', border: task.is_completed ? 'none' : '2px solid rgba(255,255,255,.18)', color: '#031008' }}>{task.is_completed && <Check className="h-4 w-4" />}</button>
              ...
              <div key={s.title} className="flex items-center gap-4 rounded-xl px-4 py-4" style={{ background: 'rgba(255,255,255,.028)', border: '1px solid rgba(255,255,255,.06)' }}>
                <Sparkles className="h-5 w-5" style={{ color: '#26d36e' }} />
                <div className="flex-1"><p className="font-bold">{s.title}</p><p className="text-xs" style={{ color: 'var(--text-muted)' }}>{s.notes}</p></div>
                <button onClick={() => addSuggestion(s)} className="rounded-lg px-3 py-2 text-xs font-black" style={{ background: 'rgba(38,211,110,.12)', color: '#26d36e', border: '1px solid rgba(38,211,110,.24)' }}>Add</button>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1fr_1fr]">
          <div className="p-5" style={cardStyle()}>
            ...
            <button onClick={() => setAllTasksOpen(true)} className="mx-auto mt-7 flex items-center gap-2 text-sm font-bold" style={{ color: '#26d36e' }}>View all tasks <ArrowRight className="h-4 w-4" /></button>
          </div>
          ...
          <div className="space-y-4">
            <div className="p-6" style={cardStyle({ minHeight: 278 })}>
              ...
              <div className="flex items-center gap-9">
                <div className="grid h-44 w-44 place-items-center rounded-full" style={{ border: '13px solid #26d36e', boxShadow: 'inset 0 0 0 1px rgba(255,255,255,.05)' }}><div className="text-center"><p className="text-3xl font-black">{doneCount}/{totalCount}</p><p>Tasks</p></div></div>
                <div className="flex-1 space-y-5 text-sm">
                  <div className="flex justify-between"><span>Completed</span><b style={{ color: '#26d36e' }}>{doneCount}</b></div>
                  ...
                  <div className="flex justify-between"><span>Completion Rate</span><b style={{ color: '#26d36e' }}>{pct}%</b></div>
                </div>
              </div>
            </div>
            <div className="p-6" style={cardStyle()}>
              ...
              <div className="flex items-center gap-8"><div><span className="text-4xl font-black" style={{ color: '#26d36e' }}>{doneCount > 0 ? 1 : 0}</span><span className="ml-3 text-xl">day</span><p className="text-sm" style={{ color: 'var(--text-muted)' }}>{doneCount > 0 ? 'Keep it going!' : 'Complete a task to start.'}</p></div><div className="flex gap-4">{['M','T','W','T','F','S'].map((d,i)=><div key={i} className="text-center"><div className="mb-2 grid h-8 w-8 place-items-center rounded-full" style={{ border: '1px solid rgba(255,255,255,.18)', background: i===0 && doneCount>0 ? '#26d36e' : 'transparent', color: i===0 && doneCount>0 ? '#031008' : 'var(--text-muted)' }}>{i===0 && doneCount>0 ? <Check className="h-4 w-4" /> : ''}</div><span className="text-xs" style={{ color: 'var(--text-muted)' }}>{d}</span></div>)}</div></div>
            </div>
          </div>
        </div>
      </div>
