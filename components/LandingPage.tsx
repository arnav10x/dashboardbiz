...
import { TrendingUp, Users, Brain, BarChart3, Target, Star, Check, ArrowRight, Flame, Shield, Clock, Activity, Zap, Trophy, CalendarCheck } from 'lucide-react'
...
function use3DTilt(str = 10) {
  ...
  useEffect(() => {
    ...
    const mv = (e: MouseEvent) => {
      ...
      const x = ((e.clientX - r.left) / r.width - 0.5) * str
      const y = ((e.clientY - r.top) / r.height - 0.5) * -str
...
function DashboardMockup() {
  return (
    <div style={{ width: 680, borderRadius: 16, border: '1px solid rgba(139,92,246,0.28)', overflow: 'hidden', boxShadow: '0 0 0 1px rgba(255,255,255,.8), 0 60px 120px rgba(0,0,0,.18), 0 0 80px rgba(139,92,246,.1), inset 0 1px 0 rgba(255,255,255,1)', background: '#080b0d', fontFamily: 'system-ui,sans-serif', fontSize: 11, position: 'relative' }}>
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 50% 0%,rgba(139,92,246,.07) 0%,transparent 60%)', pointerEvents: 'none', zIndex: 1 }} />
        ...
        <div style={{ width: 108, background: '#060809', borderRight: '1px solid rgba(255,255,255,.04)', padding: '10px 0', flexShrink: 0 }}>
          ...
          {['AI Coach', 'Team', 'Integrations'].map(l => (
            <div key={l} style={{ padding: '5px 12px', display: 'flex', alignItems: 'center', gap: 7 }}>
              <div style={{ width: 12, height: 12, borderRadius: 3, background: 'rgba(255,255,255,.05)' }} />
              <span style={{ fontSize: 10, color: '#2d3748' }}>{l}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{ flex: 1, padding: '11px 13px', display: 'flex', flexDirection: 'column', gap: 8, overflowY: 'hidden' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr', gap: 6, flex: 1, minHeight: 0 }}>
            ...
            <div style={{ background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.05)', borderRadius: 8, padding: '9px 11px' }}>
              ...
              {[{ d: true, t: 'Develop lead gen strategy' }, { d: true, t: 'Identify potential leads' }, { d: false, t: 'Add first real lead' }, { d: false, t: 'Send 10 cold DMs' }].map((tk, i) => (
...
function ExecutionTrack() {
  const PHASES = [
    {
      ...
      Icon: CalendarCheck,
      accent: '#8b5cf6',
      accentBg: 'rgba(139,92,246,.07)',
      mockup: (
        <div style={{ padding: '14px 16px', background: '#f8fafc', borderRadius: 10, border: '1px solid rgba(0,0,0,.06)' }}>
          <div style={{ fontSize: 9, color: '#94a3b8', fontWeight: 700, letterSpacing: '.1em', marginBottom: 10 }}>ONBOARDING  ·  STEP 2 OF 3</div>
          ...
          <div style={{ marginTop: 10, padding: '5px 10px', background: 'linear-gradient(135deg,#8b5cf6,#6d28d9)', borderRadius: 6, fontSize: 10, fontWeight: 700, color: '#fff', textAlign: 'center', boxShadow: '0 4px 12px rgba(139,92,246,.3)' }}>Generate my roadmap</div>
        </div>
      ),
    },
    {
      phase: 'EXECUTE',
      timeframe: 'Days 2 – 29  ·  Daily',
      ...
      Icon: Zap,
      accent: '#7c3aed',
      accentBg: 'rgba(124,58,237,.07)',
      mockup: (
        <div style={{ padding: '14px 16px', background: '#f8fafc', borderRadius: 10, border: '1px solid rgba(0,0,0,.06)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            <div style={{ fontSize: 9, color: '#94a3b8', fontWeight: 700, letterSpacing: '.1em' }}>TODAY'S TASKS</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '2px 8px', background: 'rgba(124,58,237,.08)', borderRadius: 9999, border: '1px solid rgba(124,58,237,.2)' }}>
              <Flame size={9} color="#7c3aed" />
              <span style={{ fontSize: 8.5, fontWeight: 800, color: '#7c3aed' }}>14 day streak</span>
            </div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {[{ done: true, t: 'Send 15 cold DMs to e-com founders', pts: '+50 pts' }, { done: true, t: 'Follow up: Alex R. (warm lead, 7 days)', pts: '+30 pts' }, { done: false, t: 'Log today\'s P&L entry', pts: '+20 pts' }].map((tk, i) => (
              <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '6px 8px', background: '#fff', borderRadius: 7, border: `1px solid ${tk.done ? 'rgba(139,92,246,.15)' : 'rgba(0,0,0,.07)'}` }}>
                <div style={{ width: 14, height: 14, borderRadius: 4, background: tk.done ? '#8b5cf6' : 'transparent', border: tk.done ? 'none' : '1.5px solid #cbd5e1', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: tk.done ? '0 0 8px rgba(139,92,246,.5)' : 'none' }}>
                  {tk.done && <Check size={8} color="#fff" strokeWidth={3} />}
                </div>
                <span style={{ fontSize: 9, color: tk.done ? '#94a3b8' : '#0f172a', textDecoration: tk.done ? 'line-through' : 'none', flex: 1, lineHeight: 1.3 }}>{tk.t}</span>
    ...
    },
    {
      ...
      Icon: Trophy,
      accent: '#6d28d9',
      accentBg: 'rgba(109,40,217,.07)',
      mockup: (
        <div style={{ padding: '14px 16px', background: '#f8fafc', borderRadius: 10, border: '1px solid rgba(0,0,0,.06)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <div style={{ fontSize: 9, color: '#94a3b8', fontWeight: 700, letterSpacing: '.1em' }}>PIPELINE</div>
            <div style={{ fontSize: 9, color: '#8b5cf6', fontWeight: 700 }}>1 deal just closed</div>
          </div>
          ...
          <div style={{ padding: '6px 8px', background: 'rgba(139,92,246,.06)', border: '1px solid rgba(139,92,246,.18)', borderRadius: 7 }}>
            <div style={{ fontSize: 7.5, fontWeight: 800, color: '#8b5cf6', letterSpacing: '.06em', marginBottom: 2 }}>AI COACH</div>
  ...
  return (
    <div style={{ maxWidth: 1160, margin: '0 auto' }}>
      {/* Progress rail */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 0, marginBottom: 56, position: 'relative' }}>
        <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: '60%', height: 1, background: 'linear-gradient(90deg, rgba(139,92,246,.2), rgba(139,92,246,.6), rgba(109,40,217,.2))' }} />
        {PHASES.map((p, i) => (
      ...
      <div className="lp-exec-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 0 }}>
        {PHASES.map((p, i) => (
          <TiltCard key={p.phase} style={{ borderRadius: i === 0 ? '18px 0 0 18px' : i === 2 ? '0 18px 18px 0' : '0', overflow: 'hidden' }}>
            <div className={`lp-reveal lp-d${i + 1}`} style={{
              background: '#fff',
              borderTop: `1px solid rgba(0,0,0,.07)`,
              borderBottom: `1px solid rgba(0,0,0,.07)`,
              borderLeft: i === 0 ? `1px solid rgba(0,0,0,.07)` : `1px solid rgba(139,92,246,.12)`,
  ...
  return (
    <div style={{ background: '#ffffff', color: '#0f172a', minHeight: '100vh', overflowX: 'hidden' }}>
      ...
      <style>{`
        ...
        .lp-glow-btn::after{content:'';position:absolute;inset:0;background:linear-gradient(135deg,rgba(255,255,255,.18) 0%,transparent 55%);pointer-events:none}
        .lp-glow-btn:hover{box-shadow:0 0 28px rgba(139,92,246,.35),0 10px 24px rgba(139,92,246,.18)!important;transform:translateY(-2px)!important}
      ...
      <div style={{ position: 'fixed', inset: 0, background: 'radial-gradient(ellipse 80% 60% at 50% 0%, rgba(237,233,254,.55) 0%, rgba(255,255,255,.0) 100%)', pointerEvents: 'none', zIndex: 0 }} />
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 200, height: 64, padding: '0 28px', display: 'flex', alignItems: 'center', background: scrolled ? 'rgba(255,255,255,0.88)' : 'transparent', backdropFilter: scrolled ? 'blur(24px)' : 'none', WebkitBackdropFilter: scrolled ? 'blur(24px)' : 'none', borderBottom: scrolled ? '1px solid rgba(0,0,0,.08)' : '1px solid transparent', transition: 'all .3s ease' }}>
        ...
        <div style={{ flex: 1, display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          {isLoggedIn ? (
            <Link href="/dashboard" className="lp-glow-btn" style={{ padding: '8px 20px', borderRadius: 8, background: 'linear-gradient(135deg,#8b5cf6,#6d28d9)', color: '#fff', fontSize: 14, fontWeight: 700, textDecoration: 'none', boxShadow: '0 4px 16px rgba(139,92,246,.3)' }}>Go to Dashboard →</Link>
          ) : (
            <>
              ...
              <Link href="/signup" className="lp-glow-btn" style={{ padding: '8px 20px', borderRadius: 8, background: 'linear-gradient(135deg,#8b5cf6,#6d28d9)', color: '#fff', fontSize: 14, fontWeight: 700, textDecoration: 'none', boxShadow: '0 4px 16px rgba(139,92,246,.28)' }}>Get started →</Link>
            </>
          )}
        </div>
      </nav>
      ...
      <section style={{ position: 'relative', zIndex: 1, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '130px 28px 90px', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', width: 900, height: 900, borderRadius: '50%', background: 'radial-gradient(circle, rgba(196,167,255,.22) 0%, rgba(167,139,250,.08) 40%, transparent 68%)', top: -280, right: -240, animation: 'lp-aurora 14s ease-in-out infinite', filter: 'blur(2px)' }} />
          <div style={{ position: 'absolute', width: 700, height: 700, borderRadius: '50%', background: 'radial-gradient(circle, rgba(139,92,246,.12) 0%, transparent 68%)', bottom: -200, left: -180, animation: 'lp-aurora 18s ease-in-out infinite 3s', filter: 'blur(2px)' }} />
          <div style={{ position: 'absolute', width: 500, height: 500, borderRadius: '50%', background: 'radial-gradient(circle, rgba(221,214,254,.18) 0%, transparent 68%)', top: '20%', left: '18%', animation: 'lp-aurora 12s ease-in-out infinite 5s', filter: 'blur(3px)' }} />
          {/* Dot mesh */}
          <div style={{ position: 'absolute', inset: 0, backgroundImage: 'radial-gradient(circle, rgba(139,92,246,.11) 1px, transparent 1px)', backgroundSize: '36px 36px', maskImage: 'radial-gradient(ellipse 80% 80% at 50% 50%, black 30%, transparent 100%)', WebkitMaskImage: 'radial-gradient(ellipse 80% 80% at 50% 50%, black 30%, transparent 100%)' }} />
        </div>
        ...
        <div className="lp-hero-grid" style={{ maxWidth: 1220, width: '100%', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'center' }}>
          ...
          <div>
            ...
            <div className="lp-hero-d lp-hero-btns" style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 24 }}>
              {isLoggedIn ? (
                <Link href="/dashboard" className="lp-glow-btn" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '15px 32px', borderRadius: 12, background: 'linear-gradient(135deg,#8b5cf6,#6d28d9)', color: '#fff', fontSize: 16, fontWeight: 800, textDecoration: 'none', boxShadow: '0 8px 28px rgba(139,92,246,.35),inset 0 1px 0 rgba(255,255,255,.18)' }}>
                  Go to Dashboard <ArrowRight size={16} />
                </Link>
              ) : (
                <>
                  <Link href="/signup" className="lp-glow-btn" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '15px 32px', borderRadius: 12, background: 'linear-gradient(135deg,#8b5cf6,#6d28d9)', color: '#fff', fontSize: 16, fontWeight: 800, textDecoration: 'none', boxShadow: '0 8px 28px rgba(139,92,246,.35),inset 0 1px 0 rgba(255,255,255,.18)' }}>
          ...
          <div className="lp-hero-e lp-hero-dash" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', position: 'relative' }}>
            <div style={{ position: 'absolute', width: 500, height: 300, background: 'radial-gradient(ellipse, rgba(139,92,246,.16), transparent 70%)', borderRadius: '50%', filter: 'blur(24px)', pointerEvents: 'none' }} />
            {/* Floating chips */}
            <div style={{ position: 'absolute', top: '6%', left: '-4%', zIndex: 20, animation: 'lp-float 5s ease-in-out infinite .5s' }}>
              <div style={{ padding: '8px 14px', background: 'rgba(255,255,255,.92)', backdropFilter: 'blur(20px)', border: '1px solid rgba(139,92,246,.2)', borderRadius: 12, boxShadow: '0 16px 40px rgba(0,0,0,.1)', display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 32, height: 32, borderRadius: 9, background: 'rgba(139,92,246,.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(139,92,246,.18)' }}><Activity size={14} color="#8b5cf6" /></div>
                <div><div style={{ fontSize: 14, fontWeight: 800, color: '#0f172a', lineHeight: 1 }}>+32%</div><div style={{ fontSize: 10, color: '#64748b', marginTop: 1 }}>Revenue MoM</div></div>
              </div>
            </div>
            <div style={{ position: 'absolute', bottom: '12%', right: '-6%', zIndex: 20, animation: 'lp-float 6s ease-in-out infinite 1.2s' }}>
              <div style={{ padding: '8px 14px', background: 'rgba(255,255,255,.92)', backdropFilter: 'blur(20px)', border: '1px solid rgba(139,92,246,.18)', borderRadius: 12, boxShadow: '0 16px 40px rgba(0,0,0,.1)', display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 32, height: 32, borderRadius: 9, background: 'rgba(139,92,246,.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(139,92,246,.18)' }}><Brain size={14} color="#8b5cf6" /></div>
                <div><div style={{ fontSize: 14, fontWeight: 800, color: '#0f172a', lineHeight: 1 }}>AI Coach</div><div style={{ fontSize: 10, color: '#8b5cf6', marginTop: 1 }}>Active now</div></div>
              </div>
            </div>
            <div style={{ position: 'absolute', top: '42%', right: '-8%', zIndex: 20, animation: 'lp-float 7s ease-in-out infinite 2s' }}>
              <div style={{ padding: '8px 14px', background: 'rgba(255,255,255,.92)', backdropFilter: 'blur(20px)', border: '1px solid rgba(139,92,246,.18)', borderRadius: 12, boxShadow: '0 16px 40px rgba(0,0,0,.1)', display: 'flex', alignItems: 'center', gap: 8 }}>
                <Flame size={18} color="#8b5cf6" />
                <div><div style={{ fontSize: 14, fontWeight: 800, color: '#0f172a', lineHeight: 1 }}>14 day</div><div style={{ fontSize: 10, color: '#64748b', marginTop: 1 }}>Streak</div></div>
              </div>
            </div>
            <div style={{ animation: 'lp-float3d 7s ease-in-out infinite', transformOrigin: 'center center', filter: 'drop-shadow(0 60px 80px rgba(0,0,0,.15)) drop-shadow(0 0 80px rgba(139,92,246,.1))' }}>
              <DashboardMockup />
            </div>
          </div>
        </div>
      ...
      {/* ── SOCIAL PROOF STRIP ── */}
      <div style={{ position: 'relative', zIndex: 1, borderTop: '1px solid rgba(0,0,0,.06)', borderBottom: '1px solid rgba(0,0,0,.06)', background: 'rgba(255,255,255,.82)', backdropFilter: 'blur(12px)', padding: '22px 28px' }}>
          ...
          <div className="lp-preview-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 22 }}>
            ...
            <TiltCard style={{ borderRadius: 18, overflow: 'hidden' }}>
              <div className="lp-reveal lp-d3" style={{ background: '#ffffff', border: '1px solid rgba(0,0,0,.07)', borderRadius: 18, padding: '24px 22px', position: 'relative', overflow: 'hidden', boxShadow: '0 4px 24px rgba(0,0,0,.07)', transition: 'border-color .3s,box-shadow .3s' }}>
                <div style={{ position: 'absolute', top: -40, right: -40, width: 140, height: 140, background: 'radial-gradient(circle,rgba(139,92,246,.06),transparent)', borderRadius: '50%', pointerEvents: 'none' }} />
                <span style={{ fontSize: 10, fontWeight: 800, padding: '3px 9px', background: 'rgba(139,92,246,.08)', color: '#8b5cf6', borderRadius: 9999, letterSpacing: '.05em' }}>AI COACH</span>
      ...
      {/* ── FEATURES HUB ── */}
      <section id="features" className="lp-feat-section" style={{ position: 'relative', zIndex: 1, padding: '100px 28px', background: '#f8fafc', borderTop: '1px solid rgba(0,0,0,.06)' }}>
      ...
      {/* ── AI COACH CALLOUT ── */}
      <section className="lp-ai-section" style={{ position: 'relative', zIndex: 1, padding: '100px 28px', background: '#f8fafc', borderTop: '1px solid rgba(0,0,0,.06)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div className="lp-ai-inner" style={{ background: '#ffffff', border: '1px solid rgba(139,92,246,.12)', borderRadius: 22, padding: '60px 64px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center', position: 'relative', overflow: 'hidden', boxShadow: '0 4px 40px rgba(139,92,246,.06)' }}>
