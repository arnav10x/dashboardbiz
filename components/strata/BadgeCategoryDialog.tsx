    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,.78)' }} onClick={e => e.currentTarget === e.target && setOpen(false)}>
          <div className="w-full max-w-2xl overflow-hidden rounded-2xl" style={{ background: 'var(--modal-bg)', border: '1px solid var(--border-strong)', boxShadow: '0 30px 80px rgba(0,0,0,.7)', color: 'var(--text-primary)' }}>
            <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: '1px solid var(--divider)' }}>
              <div>
                ...
                <p className="mt-1 text-xs" style={{ color: 'var(--text-muted)' }}>{earnedCount} of {items.length} unlocked. Complete real actions to earn these.</p>
              </div>
            ...
            <div className="max-h-[70vh] overflow-y-auto p-4">
              <div className="grid gap-3 sm:grid-cols-2">
                {items.map(item => {
                  ...
                  return (
                    <div key={item.id} className="rounded-xl p-4 transition hover:-translate-y-0.5" style={{ background: isEarned ? `${color}10` : 'var(--overlay-micro)', border: `1px solid ${isEarned ? `${color}44` : 'var(--border)'}` }}>
                      <div className="mb-3 flex items-center justify-between gap-3">
                        <div style={{ position: 'relative', width: 48, height: 52, display: 'flex', alignItems: 'center', justifyContent: 'center', filter: isEarned ? `drop-shadow(0 0 14px ${color}77)` : 'none', opacity: isEarned ? 1 : 0.5 }}>
                          <svg width="46" height="50" viewBox="0 0 100 110" style={{ position: 'absolute', inset: 0 }}>
                            ...
                            <path d="M50 6 L84 21 L77 72 Q68 94 50 106 Q32 94 23 72 L16 21 Z" fill={isEarned ? `url(#modal-${item.id})` : 'var(--overlay-faint)'} stroke={isEarned ? color : 'var(--border-strong)'} strokeWidth="5" />
                        ...
                        {isEarned ? <CheckCircle2 className="h-5 w-5" style={{ color: 'var(--accent)' }} /> : <Lock className="h-5 w-5" style={{ color: 'var(--text-muted)' }} />}
                      </div>
                      <h4 className="font-black">{item.title}</h4>
                      <p className="mt-1 text-xs leading-relaxed" style={{ color: 'var(--text-muted)' }}>{item.desc}</p>
                      <div className="mt-3 flex items-center justify-between gap-2 text-[10px] font-bold uppercase tracking-[.08em]">
                        ...
                        <span style={{ color: 'var(--text-muted)' }}>{item.condition}</span>
                      </div>
                    </div>
                  )
                })}
