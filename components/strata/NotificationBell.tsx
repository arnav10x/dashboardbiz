...
import { Bell, Target, CheckSquare, Flame, Users, X, TrendingUp, Clock, UserPlus } from 'lucide-react'
...
export function NotificationBell() {
  ...
  const respondToInvite = async (note: Notification, accepted: boolean) => {
    ...
    if (accepted && note.metadata?.workspace_id) {
      const { data: profile } = await supabase.from('user_profiles').select('full_name,username,email').eq('user_id', user.id).maybeSingle()
      await supabase.from('workspace_members').upsert({ workspace_id: note.metadata.workspace_id, user_id: user.id, role: 'Member', status: 'active' }, { onConflict: 'workspace_id,user_id' })
      try { await supabase.from('team_members').insert({ workspace_id: note.metadata.workspace_id, user_id: user.id, name: profile?.full_name || profile?.username || user.email?.split('@')[0] || 'Member', email: profile?.email || user.email || '', role: 'Member', status: 'accepted' }) } catch {}
    }
            ...
            {loading ? (
              ...
              <div className="p-2 space-y-1.5">
                {visible.map(note => {
                  ...
                  return (
                    <div key={note.id} className="relative flex items-start gap-3 p-3 rounded-xl"
