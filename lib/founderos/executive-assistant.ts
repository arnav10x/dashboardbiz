...
export function buildExecutiveBriefing(snapshot: FounderSnapshot) {
  ...
  if (noPipeline) priorities.push({ title: 'Add first real lead', reason: 'prspectve cannot calculate pipeline health until leads exist.', href: '/dashboard/pipeline', priority: noRevenue ? 'high' : 'urgent' })
  ...
  return {
    ...
    headline: priorities[0]?.title || 'prspectve is ready',
